...
@test "given a configuration with defaults, when i apply it, all default port values must have been set correctly." {
	ipvsadm -C
...
@test "given a configuration with defaults, when i apply it, all default scheduler values must have been set correctly." {
	ipvsadm -C
	...
	[[ "${output}" =~ 1\.2\.3\.4:80[[:blank:]]+wrr ]]
	[[ "${output}" =~ 5\.6\.7\.8:90[[:blank:]]+wrr ]]
	[[ "${output}" =~ 9\.10\.11\.12:80[[:blank:]]+rr ]]

	ipvsadm -C
}

@test "given a configuration with defaults, when i apply it, all default forward values must have been set correctly." {
	ipvsadm -C
...
@test "given a configuration with defaults, when i apply it, all default weight values must have been set correctly." {
	ipvsadm -C
